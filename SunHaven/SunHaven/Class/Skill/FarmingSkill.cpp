...
void FarmingSkill::init(string index, string name, string type, int tier, string description,
...
HRESULT FarmingSkill::init()
{
	Skill::init();

	_img.skillIcon = IMAGEMANAGER->findImage("농사스킬아이콘");
	_img.skillFrame = IMAGEMANAGER->findImage("농사스킬프레임");
...
void FarmingSkill::release()
{

}

void FarmingSkill::update()
{
	for (int i = 0; i < 40; i++)
	{
